...
function Main()
  ...
  if (assembly_markers() == 3) then
    ...
    local adaptive_delete = r.NamedCommandLookup("_XENAKIOS_TSADEL")
    r.Main_OnCommand(adaptive_delete, 0)
    local paste = r.NamedCommandLookup("_SWS_AWPASTE")
    r.Main_OnCommand(paste, 0)
    unlock_items()
    ...
    r.ShowMessageBox("Please use 3 markers: DEST-IN, SOURCE-IN and SOURCE-OUT", "Classical Assembly Line", 0)
  end
  r.Undo_EndBlock('Classical Assembly Line', 0)
  r.PreventUIRefresh(-1)
  r.UpdateArrange()
  r.UpdateTimeline()
end

function assembly_markers()
